...
void SPI1_Config(void) {
	...
	/* CR1 config */
	SPI1->CR1 = 0;
	SPI1->CR1 &= ~((1U << 0) | (1U << 1)); // CPOL=0, CPHA=0

	SPI1->CR1 |= (1U << 2);               // MSTR=1 (Master mode)

	SPI1->CR1 &= ~(7U << 3);
	SPI1->CR1 |= (5U << 3);   // fPCLK/64 ≈ 1.4 MHz ✅
...
void SPI1_GPIOConfig(void) {
	...
	/* MODER: set PA5,6,7 to AF (10), PB6 to Output (01) */
	...
	/* OSPEEDR: High speed for PA5,6,7, PB6 (11) */
	...
	/* PB6 output default HIGH (CS inactive) */
	GPIOB->BSRR = (1U << SPI1_CS_PIN);
}
