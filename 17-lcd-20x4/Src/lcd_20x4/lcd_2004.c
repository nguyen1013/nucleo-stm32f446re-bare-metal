...
// DDRAM start address for LCD 20x4 (common HD44780 mapping)
...
static void lcd2004_write4bytes(uint8_t upperNibble, uint8_t lowerNibble, uint8_t rs)
{
    uint8_t data[4];

    // EN high then low for upper nibble
    data[0] = upperNibble | LCD_BL | LCD_EN | (rs ? LCD_RS : 0);
    data[1] = upperNibble | LCD_BL |          (rs ? LCD_RS : 0);

    // EN high then low for lower nibble
    data[2] = lowerNibble | LCD_BL | LCD_EN | (rs ? LCD_RS : 0);
    data[3] = lowerNibble | LCD_BL |          (rs ? LCD_RS : 0);

    (void)I2C1_Write_LCD(LCD2004_I2C_ADDR, 4, data);
}

// Send raw 4-bit nibble (used for robust init sequence)
static void lcd2004_sendNibble(uint8_t nibble, uint8_t rs)
{
    uint8_t data[2];
    uint8_t upper = (uint8_t)(nibble & 0xF0);

    data[0] = upper | LCD_BL | LCD_EN | (rs ? LCD_RS : 0);
    data[1] = upper | LCD_BL |          (rs ? LCD_RS : 0);


    (void)I2C1_Write_LCD(LCD2004_I2C_ADDR, 2, data);
}

void LCD2004_SendCommand(uint8_t cmd)
{
    uint8_t nibble_l = cmd & 0xF0;
    uint8_t nibble_r = (uint8_t)((cmd << 4) & 0xF0);
    lcd2004_write4bytes(nibble_l, nibble_r, 0);

    // Datasheet timing: clear/home need longer delay
...
void LCD2004_SendChar(uint8_t c)
{
    ...
    lcd2004_write4bytes(nibble_l, nibble_r, 1);
}
...
void LCD2004_Init(void)
{
    ...
    // Robust 4-bit init sequence (common best practice)
    // Send 0x3 three times (as nibble) then 0x2 (set 4-bit mode)
    lcd2004_sendNibble(0x30, 0); delay_ms_tim6(5);
    lcd2004_sendNibble(0x30, 0); delay_ms_tim6(1);
    lcd2004_sendNibble(0x30, 0); delay_ms_tim6(1);
    lcd2004_sendNibble(0x20, 0); delay_ms_tim6(1); // 4-bit mode

    // Function set: 4-bit, 2-line (HD44780 uses 2-line mode even for 4 lines), 5x8 dots
    LCD2004_SendCommand(0x28);

    // Display off (optional but recommended)
...
{
    if (row >= LCD2004_ROWS) row = 0;
    if (col >= LCD2004_COLS) col = 0;
